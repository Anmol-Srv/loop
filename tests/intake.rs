...
async fn intake_needs_the_capability_and_the_owner_toggles_it(pool: PgPool) {
    ...
    assert!(message(&v).contains("Creates tasks for me"), "{v}");
    let (status, _) = send(
        &pool,
        "GET",
        "/api/agent/intake/recent",
        &w.agent,
        Value::Null,
    )
    .await;
...
async fn a_filing_is_standalone_and_wears_its_sources_label(pool: PgPool) {
    ...
    assert_eq!(
        a["labels"][0]["id"], b["labels"][0]["id"],
        "one Slack label, made once"
    );
    ...
    let seen = ok(
        &pool,
        "GET",
        &format!("/api/user/tasks/{}", a["id"].as_str().unwrap()),
        &w.other,
        Value::Null,
    )
    .await;
    ...
    let row = ok(
        &pool,
        "POST",
        &format!("/api/user/tasks/{}/accept", a["id"].as_str().unwrap()),
        &w.owner,
        Value::Null,
    )
    .await;
...
async fn appending_is_for_your_own_filings_and_dedupes_too(pool: PgPool) {
    ...
    let note = ok(
        ...
        &uri,
        &w.agent,
        json!({ "source": source("m2", "C123"), "text": "Also on web" }),
    )
    .await;
    ...
    let (status, v) = send(
        &pool,
        "POST",
        &format!("/api/agent/intake/{other_id}/append"),
        &w.agent,
        json!({ "source": source("m9", "C123") }),
    )
    .await;
    assert_eq!(status, StatusCode::FORBIDDEN);
    ...
    let note = ok(
        ...
        &uri,
        &w.agent,
        json!({ "source": source("m3", "D42") }),
    )
    .await;
    assert!(
        !note["body"].as_str().unwrap().contains("Checkout fails"),
        "{note}"
    );
}
...
async fn triage_is_accepted_or_dismissed_and_nothing_else(pool: PgPool) {
    ...
    assert!(
        tracks["eng"]
            ...
            .unwrap()
            .values()
            .all(|to| !to.as_array().unwrap().contains(&json!("triage"))),
    ...
    let (status, v) = send(
        &pool,
        "POST",
        &format!("/api/user/tasks/{ida}/accept"),
        &w.other,
        Value::Null,
    )
    .await;
    ...
    assert_eq!(
        status,
        StatusCode::FORBIDDEN,
        "an agent cannot accept its own filing"
    );
    ...
    let (status, v) = send(
        &pool,
        "POST",
        &format!("/api/user/tasks/{ida}/dismiss"),
        &w.owner,
        Value::Null,
    )
    .await;
    ...
    let agents = ok(&pool, "GET", "/api/user/agents", &w.owner, Value::Null).await;
    assert_eq!(
        agents[0]["intakeStats"],
        json!({ "triage": 0, "accepted": 1, "dismissed": 1 })
    );
}
...
async fn triage_shows_in_counts_and_home_and_not_as_open_work(pool: PgPool) {
    ...
    ok(
        &pool,
        "PATCH",
        &format!("/api/user/tasks/{id}/details"),
        &w.other,
...
async fn a_direct_messages_words_are_the_owners(pool: PgPool) {
    ...
    let get = |id: &Value, who: &str| {
        let (pool, who, uri) = (
            pool.clone(),
            who.to_owned(),
            format!("/api/user/tasks/{}", id.as_str().unwrap()),
        );
        async move { ok(&pool, "GET", &uri, &who, Value::Null).await }
    };
    let seen = get(&public["id"], &w.other).await;
    assert_eq!(
        seen["source"]["text"], "Checkout fails for saved cards",
        "a channel message is team-visible"
    );
    ...
    for task in [&dm, &flagged] {
        ...
        assert_eq!(
            teammate["source"]["url"], task["source"]["url"],
            "the rest is team-visible"
        );
    ...
    let private: Vec<&Value> = all
        .as_array()
        .unwrap()
        .iter()
        .filter(|t| t["source"]["private"] == true)
        .collect();
    assert_eq!(private.len(), 2);
    assert!(
        private
            .iter()
            .all(|t| t["source"]["author"].is_null()
                && t["source"]["text"] == "From a direct message")
    );
}
...
async fn recent_lists_what_this_agent_filed(pool: PgPool) {
    ...
    ok(
        &pool,
        "POST",
        &format!("/api/agent/intake/{}/append", b["id"].as_str().unwrap()),
        &w.agent,
        json!({ "source": source("m3", "C123") }),
    )
    .await;
    sqlx::query(
        "UPDATE task_source SET created_at = now() - interval '20 days' WHERE source_key = 'm1'",
    )
    .execute(&pool)
    .await
    .unwrap();
    ...
    assert_eq!(
        ...
        1,
        "14 days by default, appends are not filings: {recent}"
    );
    ...
    assert_eq!(
        recent[0]["source"],
        json!({ "key": "m2", "url": "https://slack.test/m2", "channel": "C123" })
    );
    let month = ok(
        &pool,
        "GET",
        "/api/agent/intake/recent?days=30",
        &w.agent,
        Value::Null,
    )
    .await;
...
async fn the_intake_tools_are_there_only_with_the_capability(pool: PgPool) {
    ...
    assert!(
        !before.iter().any(|n| n.starts_with("intake_")),
        "{before:?}"
    );
    let r = rpc(
        &pool,
        ...
        2,
        "tools/call",
        json!({ "name": "intake_recent", "arguments": {} }),
    )
    .await;
    ...
    ok(
        &pool,
        "PATCH",
        &format!("/api/user/agents/{}", w.agent_id),
        &w.owner,
        json!({ "canIntake": true }),
    )
    .await;
    ...
    let r = rpc(
        ...
        4,
        "tools/call",
        json!({ "name": "intake_create", "arguments": filing("m1", "C1") }),
    )
    .await;
    ...
    assert!(tool_text(&r)
        .as_str()
        .unwrap()
        .contains(task["id"].as_str().unwrap()));
    let r = rpc(
        &pool,
        &w.agent,
        6,
        "tools/call",
        json!({ "name": "intake_create", "arguments": { "title": "x" } }),
    )
    .await;
    assert_eq!(r["result"]["isError"], true);
    assert!(
        tool_text(&r).as_str().unwrap().contains("missing field"),
        "{r}"
    );
    ...
    let r = rpc(
        ...
        7,
        "tools/call",
        json!({ "name": "intake_append",
        "arguments": { "taskId": task["id"], "source": source("m2", "C1"), "text": "again" } }),
    )
    .await;
    ...
    let r = rpc(
        ...
        8,
        "tools/call",
        json!({ "name": "intake_recent", "arguments": { "days": 7 } }),
    )
    .await;
...
async fn filing_leaves_the_inbox_alone(pool: PgPool) {
    ...
    ok(
        &pool,
        "POST",
        &format!("/api/agent/intake/{}/append", t["id"].as_str().unwrap()),
        &w.agent,
        json!({ "source": source("m2", "C1") }),
    )
    .await;
    ok(
        &pool,
        "POST",
        &format!("/api/user/tasks/{}/accept", t["id"].as_str().unwrap()),
        &w.owner,
        Value::Null,
    )
    .await;
    assert_eq!(
        inbox(pool.clone(), w.agent.clone()).await,
        before,
        "byte-stable"
    );
}
