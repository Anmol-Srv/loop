...
use super::agents::{runtime_label, setup_checks, state_tone, state_words, status_words, Opened};
...
pub(super) fn show(
    ...
    let agent = overview.as_deref().and_then(|o| o.get("agent")).cloned().or(listed).unwrap_or(Value::Null);
    let mut ask = None;

    let back_label = if opened.back.is_some() { "Back" } else { "All agents" };
...
fn header(ui: &mut egui::Ui, a: &Value, owned: bool, seed: &str, name: &str, short: &str) -> Option<Ask> {
    ...
    ui.horizontal_top(|ui| {
        ...
        match status {
            "connected" => face::avatar(ui, seed, face::XL, Presence::of(state, str_of(a, "lastSeenAt")), name),
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Min), |ui| {
                ...
                viz::more(ui, |ui| {
                    ...
                    if status != "revoked" {
                        ...
                        if viz::menu_item(ui, label, false, None) {
                            ask = Some(Ask::Card(name.to_owned(), CardAsk::Intake(!intake)));
                        }
            ...
            ui.with_layout(egui::Layout::top_down(egui::Align::Min), |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    let mut who = format!("\u{00B7} {}", runtime_label(str_of(a, "runtime").unwrap_or("other")));
                    if owned {
...
fn now_line(ui: &mut egui::Ui, a: &Value, current: Option<&Value>, state: &str, status: &str, short: &str) -> Option<String> {
    let mut open = None;
    ui.horizontal(|ui| {
        ...
        match (status, current) {
            ...
            ("waiting", _) => {
                ui.label(RichText::new("Waiting for first contact").size(text::SMALL).color(colour::WARN));
            }
            (_, Some(t)) => {
                ...
                let ink = if state == "needs_input" { colour::WARN } else { colour::TEXT_2 };
                ui.label(RichText::new(words).size(text::BODY).color(ink));
                ui.label(RichText::new("on").size(text::SMALL).color(colour::TEXT_MUTED));
                if task_link(ui, str_of(t, "title").unwrap_or("Untitled")) {
                    open = str_of(t, "id").map(str::to_owned);
                }
            }
            _ => {
                let seen = str_of(a, "lastSeenAt").map(|s| format!(" \u{00B7} seen {}", ago(s))).unwrap_or_default();
                let intake = a.get("canIntake").and_then(Value::as_bool).unwrap_or(false);
                let next = if intake { String::new() } else { format!(" Hand {short} one from any task\u{2019}s page.") };
...
fn rail(ui: &mut egui::Ui, a: &Value, owned: bool) -> Option<CardAsk> {
    ...
    shell::property(ui, "Last seen", |ui| match str_of(a, "lastSeenAt") {
        Some(at) => {
            ui.label(RichText::new(ago(at)).size(text::SMALL).color(colour::TEXT_2)).on_hover_text(exact(at));
        }
        None => w::muted(ui, "Never"),
    });
    shell::property(ui, "Connected", |ui| match str_of(a, "connectedAt") {
        Some(at) => {
            ui.label(RichText::new(day_label(at)).size(text::SMALL).color(colour::TEXT_2)).on_hover_text(exact(at));
        }
        None => w::muted(ui, "Not yet"),
    });
    shell::property(ui, "Runtime", |ui| {
        ui.label(RichText::new(runtime_label(str_of(a, "runtime").unwrap_or("other"))).size(text::SMALL).color(colour::TEXT_2));
    });
    shell::property(ui, "Handle", |ui| {
        ui.label(RichText::new(str_of(a, "handle").unwrap_or_default()).monospace().size(text::SMALL).color(colour::TEXT_2));
    });
    shell::property(ui, "Intake", |ui| {
        let words = if intake { "Creates tasks for you" } else { "Off" };
        ui.label(RichText::new(words).size(text::SMALL).color(if intake { colour::TEXT_2 } else { colour::TEXT_MUTED }));
    });
    ...
    ui.add_space(space::XS);
    match a.get("setup").filter(|s| s.as_object().is_some_and(|o| !o.is_empty())) {
        Some(setup) => {
            ui.spacing_mut().item_spacing.y = space::XS;
            setup_checks(ui, setup, false);
        }
        None => w::caption(ui, "What it installed shows here once it says hello: its skill, MCP server and watcher."),
    }
    ...
    if intake {
        ui.add_space(space::MD);
        w::caption(ui, "Files what it reads into your Triage, each task labelled Slack.");
    }
    ask
}
...
fn facts(ui: &mut egui::Ui, a: &Value, owned: bool) -> Option<CardAsk> {
    ...
    ui.horizontal_wrapped(|ui| {
        ...
        if let Some(at) = str_of(a, "lastSeenAt") {
            ui.label(RichText::new(format!("seen {}", ago(at))).size(text::SMALL).color(colour::TEXT_MUTED)).on_hover_text(exact(at));
        }
        ui.label(RichText::new(runtime_label(str_of(a, "runtime").unwrap_or("other"))).size(text::SMALL).color(colour::TEXT_MUTED));
        if let Some(setup) = a.get("setup").filter(|s| s.as_object().is_some_and(|o| !o.is_empty())) {
...
fn body(ui: &mut egui::Ui, o: &Value, a: &Value, tab: &mut usize, owned: bool, seed: &str, short: &str) -> Option<Ask> {
    let intake = a.get("canIntake").and_then(Value::as_bool).unwrap_or(false);
    let stats = o.get("stats").cloned().unwrap_or(Value::Null);
    let arr = |k: &str| o.get(k).and_then(Value::as_array).map_or(&[][..], Vec::as_slice);
    let active = o.get("tasks").and_then(|t| t.get("active")).and_then(Value::as_array).map_or(&[][..], Vec::as_slice);
    let recent_tasks = o.get("tasks").and_then(|t| t.get("recent")).and_then(Value::as_array).map_or(&[][..], Vec::as_slice);
    let mut ask = None;
...
fn figures(ui: &mut egui::Ui, s: &Value, intake: bool) {
    ...
    if worker {
        cells.push((n("tasksHandled").to_string(), "Handed to it", "Tasks ever handed to it."));
        cells.push((n("tasksDone").to_string(), "Approved", "Tasks you approved after its report."));
        cells.push((n("inReviewNow").to_string(), "In review", "Reports waiting on your review now."));
        if !filer {
            cells.push((n("questionsAsked").to_string(), "Questions", "Questions it asked you."));
        }
        let ack = s.get("medianAckMinutes").and_then(Value::as_f64).map_or_else(|| "\u{2014}".to_owned(), minutes);
        cells.push((ack, "First reply", "Median time from hand-off to its first update or log line."));
    }
    if filer {
        cells.push((n("filed").to_string(), "Filed", "Tasks it filed into your Triage."));
        if !worker {
            cells.push((n("accepted").to_string(), "Accepted", "Filed tasks you accepted."));
            cells.push((n("dismissed").to_string(), "Dismissed", "Filed tasks you dismissed."));
        }
        let rate = s.get("acceptRate").and_then(Value::as_f64).map_or_else(|| "\u{2014}".to_owned(), |r| format!("{:.0}%", r * 100.0));
        cells.push((rate, "Accept rate", "Of the filed tasks you decided on, how many you accepted."));
    }
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ...
            let value_font = egui::FontId::new(text::TITLE, egui::FontFamily::Name(theme::SEMIBOLD.into()));
            let label_font = egui::FontId::proportional(text::SMALL);
            let (rect, _) = ui.allocate_exact_size(vec2(width, text::TITLE + text::SMALL + space::SM), egui::Sense::hover());
            for (i, (value, label, hint)) in cells.iter().enumerate() {
                let cell = egui::Rect::from_min_size(pos2(rect.left() + i as f32 * cell_w, rect.top()), vec2(cell_w, rect.height()));
                ...
                p.text(pos2(x, cell.top()), egui::Align2::LEFT_TOP, value, value_font.clone(), colour::TEXT);
                let label_g = w::truncated(ui, label, label_font.clone(), colour::TEXT_MUTED, cell_w - pad::CARD.0 * 2.0);
                ui.painter().galley(pos2(x, cell.bottom() - label_g.size().y), label_g, colour::TEXT_MUTED);
                if i > 0 {
                    ui.painter().vline(cell.left(), cell.y_range().expand(space::XXS), egui::Stroke::new(1.0, colour::LINE));
                }
            }
        });
}
...
fn chart(ui: &mut egui::Ui, days: &[Value]) {
    ...
    let totals = ["notes", "filed", "events"].map(|k| days.iter().map(|d| n(d, k)).sum::<f32>() as i64);
    let series = [("Its updates", colour::AGENT, "notes"), ("Filed", colour::INFO, "filed"), ("Sent to it", colour::IDLE, "events")];
    ...
    c::surface(ui, false, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                for (j, &i) in shown.iter().enumerate().rev() {
                    ...
                    ui.label(RichText::new(totals[i].to_string()).size(text::SMALL).family(egui::FontFamily::Name(theme::SEMIBOLD.into())).color(colour::TEXT));
                    ui.label(RichText::new(*label).size(text::SMALL).color(colour::TEXT_MUTED));
                    let (sw, _) = ui.allocate_exact_size(egui::Vec2::splat(text::SMALL), egui::Sense::hover());
                    ui.painter().rect_filled(egui::Rect::from_center_size(sw.center(), egui::Vec2::splat(8.0)), 2.0, *ink);
                }
            });
        });
        ui.add_space(space::MD);

        let (rect, response) = ui.allocate_exact_size(vec2(ui.available_width(), CHART_H), egui::Sense::hover());
        let spoken = format!(
        ...
        let peak = days.iter().map(|d| series.iter().map(|(_, _, k)| n(d, k)).sum::<f32>()).fold(1.0, f32::max);
        let hovered = response.hover_pos().map(|p| (((p.x - rect.left()) / (bar + BAR_GAP)) as usize).min(count - 1));
        let p = ui.painter();
        p.hline(rect.x_range(), rect.bottom() + 0.5, egui::Stroke::new(1.0, colour::LINE));
        for (i, d) in days.iter().enumerate() {
            let x = rect.left() + i as f32 * (bar + BAR_GAP);
            if hovered == Some(i) {
                p.rect_filled(egui::Rect::from_min_max(pos2(x - 1.0, rect.top()), pos2(x + bar + 1.0, rect.bottom())), 2.0, colour::SURFACE_HOVER);
            }
            let total: f32 = series.iter().map(|(_, _, k)| n(d, k)).sum();
            if total == 0.0 {
                p.rect_filled(egui::Rect::from_min_size(pos2(x, rect.bottom() - 2.0), vec2(bar, 2.0)), 1.0, colour::LINE_STRONG);
                continue;
            }
            ...
            for (_, ink, k) in &series {
                ...
                p.rect_filled(egui::Rect::from_min_max(pos2(x, y - h), pos2(x + bar, y)), 1.0, *ink);
                y -= h;
            }
        }
        ...
        ui.add_space(space::XS);
        ui.horizontal(|ui| {
            w::caption(ui, &days.first().and_then(|d| str_of(d, "day")).map(date_label).unwrap_or_default());
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| w::caption(ui, "Today"));
        });
    });
}
...
fn activity(ui: &mut egui::Ui, recent: &[Value], owned: bool, seed: &str, short: &str) -> Option<Ask> {
    if recent.is_empty() {
        empty(
            ui,
            "Nothing yet",
    ...
    for r in recent {
        ...
        if this_day != day {
            ...
            day = this_day;
            ui.label(RichText::new(&day).size(text::CAPTION).family(egui::FontFamily::Name(theme::MEDIUM.into())).color(colour::TEXT_MUTED));
            ui.add_space(space::SM);
        ...
        let (node, who, verb) = match kind {
            ...
            "submission" => (Node::Mark(Mark::Submitted), short, "submitted it for review"),
        ...
        let shows_text = !matches!(kind, "filed" | "handed_off" | "taken_back") && !text_body.trim().is_empty();
        ui.spacing_mut().item_spacing.y = space::XS;
    ...
    for (slot, group) in rails.into_iter().zip(groups) {
        let segments: Vec<egui::Shape> = group
            ...
            .map(|w| {
                let x = w[0].center().x;
                egui::Shape::line_segment([pos2(x, w[0].bottom() + space::XS), pos2(x, w[1].top() - space::XS)], egui::Stroke::new(1.0, colour::LINE))
            })
            .collect();
...
fn tasks(ui: &mut egui::Ui, active: &[Value], recent: &[Value], short: &str) -> Option<Ask> {
    ...
    let mut open = None;
    for (label, rows, id) in [("Active", active, "agent-page:active"), ("Recent", recent, "agent-page:recent")] {
        if rows.is_empty() {
            continue;
        }
        ui.horizontal(|ui| {
            ui.spacing_mut().item_spacing.x = space::XS;
            ui.label(RichText::new(label).size(text::SMALL).family(egui::FontFamily::Name(theme::MEDIUM.into())).color(colour::TEXT_MUTED));
            ui.label(RichText::new(rows.len().to_string()).size(text::SMALL).color(colour::TEXT_FAINT));
        });
        ui.add_space(space::SM);
...
fn runs(ui: &mut egui::Ui, runs: &[Value], intake: bool, short: &str) {
    ...
    ui.horizontal(|ui| {
        ...
        let failed = runs.iter().filter(|r| str_of(r, "status") == Some("failed")).count();
        let spoken = format!("Last {} runs, {failed} failed", runs.len());
        response.widget_info(|| egui::WidgetInfo::labeled(egui::WidgetType::Image, true, &spoken));
        for (i, r) in runs.iter().rev().enumerate() {
            let x = rect.left() + i as f32 * (TICK_W + 2.0);
            ui.painter().rect_filled(egui::Rect::from_min_size(pos2(x, rect.top()), vec2(TICK_W, TICK_H)), 1.0, run_ink(str_of(r, "status").unwrap_or("ok")));
        }
        let mut words = str_of(&runs[0], "finishedAt").map(|at| format!("Last run {}", ago(at))).unwrap_or_default();
    ...
    w::card_list(ui, |ui| {
        ...
        for (i, r) in runs.iter().enumerate() {
            if i > 0 {
                let (rule, _) = ui.allocate_exact_size(vec2(ui.available_width(), 1.0), egui::Sense::hover());
                ui.painter().hline(rule.x_range(), rule.center().y, egui::Stroke::new(1.0, colour::LINE_SOFT));
            }
            run_row(ui, r);
        }
    });
...
fn run_row(ui: &mut egui::Ui, r: &Value) {
    ...
    ui.horizontal(|ui| {
        ...
        fixed(ui, 96.0, |ui| {
            ui.label(RichText::new(ago(finished)).size(text::SMALL).color(colour::TEXT_2)).on_hover_text(exact(finished));
            if !took.is_empty() {
                ui.label(RichText::new(took).size(text::SMALL).color(colour::TEXT_FAINT));
            }
        });
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if let Some(s) = str_of(r, "summary").filter(|s| !s.is_empty()) {
                ui.add(egui::Label::new(RichText::new(s).size(text::SMALL).color(colour::TEXT_MUTED)).truncate());
            }
        });
    });
    if let Some(err) = str_of(r, "error") {
        let id = egui::Id::new(("agent-page:run-error", r.get("id").and_then(Value::as_i64).unwrap_or(0)));
        ...
        if open {
            ui.horizontal(|ui| {
                ...
                log_well(ui, |ui| {
                    ui.label(RichText::new(err).monospace().size(text::SMALL).color(colour::LOG_TEXT));
                });
            });
            ui.add_space(space::SM);
        }
...
fn counts(ui: &mut egui::Ui, c: &Value) {
    let parts: Vec<(&str, i64)> = [("filed", "filed"), ("appended", "appended"), ("alreadyFiled", "already filed"), ("skipped", "skipped")]
        .iter()
    ...
    ui.horizontal(|ui| {
        ui.spacing_mut().item_spacing.x = space::XS;
        if parts.is_empty() {
            ui.label(RichText::new("nothing new").size(text::SMALL).color(colour::TEXT_MUTED));
        }
        for (i, (word, n)) in parts.iter().enumerate() {
            ...
            ui.label(RichText::new(n.to_string()).size(text::SMALL).family(egui::FontFamily::Name(theme::SEMIBOLD.into())).color(colour::TEXT));
        }
    });
}
...
fn cadence(runs: &[Value]) -> Option<String> {
    let times: Vec<DateTime<chrono::Utc>> = runs.iter().filter_map(|r| str_of(r, "startedAt").and_then(parse)).collect();
    if times.len() < 3 {
        return None;
    }
    let mut gaps: Vec<i64> = times.windows(2).map(|w| (w[0] - w[1]).num_seconds().abs()).collect();
    gaps.sort_unstable();
...
fn logs(ui: &mut egui::Ui, lines: &[Value], short: &str) -> Option<Ask> {
    ...
    ui.horizontal(|ui| {
        w::muted(ui, &format!("The latest {} lines across its tasks, oldest first.", lines.len()));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if w::link(ui, "Copy all").clicked() {
                let all: Vec<String> = groups
                    .iter()
                    .map(|(_, title, rows)| format!("# {title}\n{}", rows.iter().map(|l| str_of(l, "text").unwrap_or_default()).collect::<Vec<_>>().join("\n")))
                    .collect();
    ...
    for (task, title, rows) in &groups {
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                if copy.clicked() {
                    let text: Vec<&str> = rows.iter().map(|l| str_of(l, "text").unwrap_or_default()).collect();
        ...
        ui.add_space(space::XS);
        let gutter = rows.iter().filter_map(|l| l.get("seq").and_then(Value::as_i64)).max().unwrap_or(0).to_string().len().max(3);
        log_well(ui, |ui| {
            ui.spacing_mut().item_spacing.y = space::XXS;
            for l in rows {
                ui.horizontal_top(|ui| {
                    ...
                    ui.add_space(space::SM);
                    ui.label(RichText::new(str_of(l, "text").unwrap_or_default()).monospace().size(text::SMALL).color(colour::LOG_TEXT));
                });
            }
        });
        ui.add_space(space::LG);
...
fn task_link(ui: &mut egui::Ui, title: &str) -> bool {
    let r = ui
        .add(egui::Label::new(RichText::new(title).size(text::SMALL).color(colour::TEXT_MUTED)).truncate().sense(egui::Sense::click()))
    ...
    if r.hovered() || r.has_focus() {
        ...
        ui.painter().hline(r.rect.x_range(), y, egui::Stroke::new(1.0, colour::TEXT_MUTED));
    }
    r.clicked()
}

fn fixed(ui: &mut egui::Ui, width: f32, add: impl FnOnce(&mut egui::Ui)) {
    ui.allocate_ui_with_layout(vec2(width, table::ROW_H), egui::Layout::left_to_right(egui::Align::Center), |ui| {
        ui.set_width(width);
        ui.spacing_mut().item_spacing.x = space::XS;
        add(ui);
    });
...
fn parse(s: &str) -> Option<DateTime<chrono::Utc>> {
    DateTime::parse_from_rfc3339(s).ok().map(|t| t.with_timezone(&chrono::Utc))
}

/// "Today", "Yesterday", or "Mon 22 Sep", in local time.
fn day_label(at: &str) -> String {
    let Some(t) = parse(at) else { return String::new() };
...
fn date_label(day: &str) -> String {
    NaiveDate::parse_from_str(day, "%Y-%m-%d").map(|d| d.format("%-d %b").to_string()).unwrap_or_default()
}
