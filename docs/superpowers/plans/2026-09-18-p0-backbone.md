...
- Local Postgres 17 is installed via Homebrew but not running. Start it once with `brew services start postgresql@17` before Task 3.
...
```
DATABASE_URL=postgres://localhost:5432/acp_dev
PORT=8080
```
...
createdb acp_dev
echo 'DATABASE_URL=postgres://localhost:5432/acp_dev' > .env
```
